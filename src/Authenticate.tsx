...
export function Authenticate({ onAuthenticated }: AuthenticateProps) {
    ...
    const pollForToken = async (readKey: string): Promise<string | null> => {
        return new Promise(resolve => {
            const interval = setInterval(async () => {
                const response = await fetch(`${AUTH_BACKEND}/poll`, {
                    method: "POST",
    ...
    const handleConnect = async () => {
        setIsLoading(true)
        try {
            // 1. Get authorization URL from our worker
            const authResponse = await fetch(`${AUTH_BACKEND}/authorize`, { method: "POST" })
            if (!authResponse.ok) throw new Error("Failed to get authorization URL")
            const { url, readKey } = await authResponse.json()

            // 2. Open the authorization URL in a new window
            window.open(url)

            // 3. Poll for the token
            const accessToken = await pollForToken(readKey)
            if (!accessToken) {
                framer.notify("Authentication failed or was cancelled.", { variant: "error" })
                return
            }

            // 4. Pass the access token to the parent component
        ...
        <main className="framer-hide-scrollbar setup">
            <div className="intro">
                ...
                <div className="content">
                    <h2>Pinterest</h2>
                    <p>Connect your Pinterest account to sync your pins.</p>
                </div>
            </div>

            <div className="form">
                <button onClick={handleConnect} disabled={isLoading}>
                    {isLoading ? <div className="framer-spinner" /> : "Connect to Pinterest"}
                </button>
            </div>
        </main>
    )
